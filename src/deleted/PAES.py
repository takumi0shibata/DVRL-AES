...
from utils_tmp import load_asap_dataset
...
for prompt in range(1, 9):
    ...
    ####################################
    # ADD
    df_target = load_asap_dataset('/home/ito/Documents/repo/DVRL-AES/data', stratify=True)
    target_id = df_target['essay_id'].to_list()
    ####################################
    ...
    target_indices = np.where(np.isin(target_data['essay_id'], target_id))[0]
    target = {
        'pos_x': target_data['pos_x'][target_indices],
        'linguistic_x': target_data['feature'][target_indices],
        'readability_x': target_data['readability'][target_indices],
        'normalized_label': target_data['scaled_score'][target_indices],
        'essay_set': target_data['essay_set'][target_indices],
        'essay_id': target_data['essay_id'][target_indices]
    }

    print(f'    Number of source samples for training: {len(source["essay_set"])}')
    print(f'    Number of source samples for validation: {len(dev["essay_set"])}')
    print(f'    Number of target samples for testing: {len(target["essay_set"])}')
    ...
    test_loader = DataLoader(dataset=target_dataset, batch_size=10, shuffle=False)
    ...
    # pl.DataFrame({'essay_id': target_data['essay_id'], 'pred': y_pred}).write_csv(f'/home/ito/Documents/repo/DVRL-AES/src/deleted/PAES_pred_{prompt}_seed{seed}.csv')
    
# pl.DataFrame({'prompt': prompts, 'qwk': qwks}).write_csv(f'/home/ito/Documents/repo/DVRL-AES/src/deleted/PAES_performance_seed{seed}.csv')
end_time = time.time()
